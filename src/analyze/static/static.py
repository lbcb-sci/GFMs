...
def word_embeddings(all_models: dict, logger) -> dict:
    ...
    for data, tok, type in DATA_TOKENIZER_PAIRS:
        ...
        logger.info(f' extracting word embeddings for run {key}...')
        ...
        for k in K_VALUES:
            ...
            results[key][f'top_{k}'] = m.topk(cosine_similarities.cpu(), k=k)

            logger.info(f' computing local spearman k={k}...')
            results[key][f'local_spearman_{k}'] = m.local_spearman(cosine_similarities.cpu(), k=k)
        ...
        results[key]['disparities'] = m.meanstd(disparities)
        results[key]['procrustes_cosine'] = m.meanstd(procrustes_cosine_similarities)

        logger.info(f' computing linear cka...')
        results[key]['linear_cka'] = m.cka(embeddings, kernel='linear')

        logger.info(f' computing rbf cka...')
        results[key]['rbf_cka'] = m.cka(embeddings, kernel='rbf')
